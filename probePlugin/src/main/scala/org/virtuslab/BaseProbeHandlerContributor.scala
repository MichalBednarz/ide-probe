...
class BaseProbeHandlerContributor extends ProbeHandlerContributor {
  ...
  override def registerHandlers(handler: ProbeHandler): ProbeHandler = {
    handler
      ...
      .on(Endpoints.ModuleDependencies)(Modules.dependencies)
      .on(Endpoints.ModuleSdk)(Modules.sdk)
      .on(Endpoints.ModuleSources)(Modules.sources)
      .on(Endpoints.ModuleResources)(Modules.resources)
      .on(Endpoints.ModuleTestSources)(Modules.testSources)
      .on(Endpoints.ModuleTestResources)(Modules.testResources)
